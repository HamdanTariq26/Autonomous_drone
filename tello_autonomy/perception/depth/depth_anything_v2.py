...
def undistort_image(image_bgr, intrinsics):
    """
        Removes the lens distortion described by intrinsics.dist_coeffs
        before the image reaches Depth-Anything-V2. The network itself has
        no intrinsics input - this is the one place real calibration can
        actually change what it sees: a clean pinhole image instead of one
        with our measured k1/k2/p1/p2 distortion baked in.
    """
    return cv2.undistort(image_bgr, intrinsics.camera_matrix, intrinsics.dist_coeffs)
...
def infer_metric_depth(image_bgr, model=None, intrinsics=None):
    """
        ...
        intrinsics: optional perception.camera_intrinsics.CameraIntrinsics.
            When given, the image is undistorted before inference (see
            undistort_image()). When None (the default - existing call
            sites are unaffected), behaves exactly as before.
    """
    if model is None:
        model = load_model()
    if intrinsics is not None:
        image_bgr = undistort_image(image_bgr, intrinsics)
    with torch.no_grad():
