...
def compute_scale_factor_from_ratios(ratios_by_keyframe, ratio_min=None, ratio_max=None):
    """
        ...
        pipeline_audit.md Finding #6: before taking the median, every
        ratio is dropped if it falls outside [ratio_min, ratio_max]
        (defaulting to constants.SCALE_RATIO_MIN / SCALE_RATIO_MAX). A
        plain median has no protection once more than ~50% of a cycle's
        points are corrupted (e.g. by motion blur on a fast yaw); this
        bounded filter catches the worst offenders - a near-zero SLAM
        depth that slipped past MIN_SLAM_DEPTH, a spurious near-zero
        Depth-Anything prediction, or a genuine outlier - without
        touching plausible-but-unusual values. This is a cheap sanity
        filter, not a statistical outlier test (no MAD/IQR trimming) -
        the median itself already provides most of the robustness; this
        just removes the ratios extreme enough that letting them into
        the pool at all was never useful.

        Returns (scale_factor, num_points_used):
          - (None, 0) if there are no usable ratios at all (either
            because none were computed, or every one was filtered out
            by the bounds check above).
          - (median_ratio, count) otherwise, where count is the number
            of ratios that survived filtering, not the raw pre-filter
            count.
    """
    ratio_min = ratio_min if ratio_min is not None else constants.SCALE_RATIO_MIN
    ratio_max = ratio_max if ratio_max is not None else constants.SCALE_RATIO_MAX

    all_ratios = [r for kf_ratios in ratios_by_keyframe for r in kf_ratios]
    filtered_ratios = [r for r in all_ratios if ratio_min <= r <= ratio_max]
    if not filtered_ratios:
        return None, 0
    return float(np.median(filtered_ratios)), len(filtered_ratios)
