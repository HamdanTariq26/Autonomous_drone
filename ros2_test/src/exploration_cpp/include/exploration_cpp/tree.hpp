...
nbvInspection::TreeBase<stateVec>::TreeBase(mesh::StlMesh * mesh,
                                            octomap_manager_shim::OctomapManagerShim * manager)
{
  mesh_ = mesh;
  manager_ = manager;
  bestGain_ = params_.zero_gain_;
...
template<typename stateVec>
void nbvInspection::TreeBase<stateVec>::evade(const tello_autonomy_msgs::msg::Segment& segmentMsg) {
  int i;
  ...
  segments_[i]->clear();
  for(typename std::vector<geometry_msgs::msg::Pose>::const_iterator it = segmentMsg.poses.begin(); it != segmentMsg.poses.end(); it++) {
