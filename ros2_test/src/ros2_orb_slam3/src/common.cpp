...
MonocularMode::MonocularMode() :Node("mono_node_cpp")
{
    ...
    this->declare_parameter("enable_pangolin", false); // Configurable Pangolin viewer window flag (default false to avoid background crash)
    ...
    enablePangolinWindow = this->get_parameter("enable_pangolin").as_bool();
...
void MonocularMode::initializeVSLAM(std::string& configString){
    ...
    // enablePangolinWindow set via enable_pangolin ROS2 parameter (default: false to prevent background crash)
    enableOpenCVWindow = true; // Shows OpenCV window output
    
    RCLCPP_INFO(this->get_logger(), "Pangolin viewer window enabled: %s", enablePangolinWindow ? "true" : "false");
...
void MonocularMode::PublishCurrentPoseAndPoints(const Sophus::SE3f& Tcw)
{
    ...
    // ---- Currently tracked map points, world-frame 3D (arbitrary SLAM units) ----
    std::vector<ORB_SLAM3::MapPoint*> trackedPoints = pAgent->GetTrackedMapPoints();
    std::vector<Eigen::Vector3f> worldPoints;
    worldPoints.reserve(trackedPoints.size());
    for (auto* mp : trackedPoints)
    {
        if (!mp || mp->isBad()) continue;
        Eigen::Vector3f worldPos = mp->GetWorldPos();
        Eigen::Vector3f camPos = Tcw * worldPos;  // camera-frame: used only for visibility check
        if (camPos.z() <= 0) continue;  // behind camera - invalid
        worldPoints.push_back(worldPos);  // publish WORLD-frame point, not camera-frame
    }
