...
// Added to get all key frames poses but mixes up map id global map id for every keyframe (Hamdan)
...
//Added:
// Returns all of the keyframes but with mixed up map id (Hamdan)
std::vector<KeyFrame*> System::GetAllKeyFrames(){
    return mpAtlas->GetCurrentMap()->GetAllKeyFrames();
}

//Added:
//Returns single keyframe id needed to expose current map id (Hamdan)
